...
# TODO: terrible. This should come from something else...
...
def get_to_pe(run_id):
    to_pe_file = straxen_dir +'/data/to_pe.json'
    
    with open(to_pe_file,'r') as f:
        to_pe_db = json.load(f)
    try: 
        to_pe = to_pe_db[run_id]
    except: 
        to_pe = to_pe_db['DEFAULT']
    return np.asarray(to_pe)
...
def get_elife(run_id):
    elife_file = straxen_dir +'/data/elife.json'
    with open(elife_file,'r') as f:
        elife_db = json.load(f)
    try: 
        elife = elife_db[run_id]
    except: 
        elife = elife_db['DEFAULT']
    return elife
