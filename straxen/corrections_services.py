...
@strax.Context.add_method
def pre_initlize_cmt_corrections(context: strax.Context, cmt_run_id):
    """
    Function which loads CMT corrections for current context and
    cmt_run_id and returns CMT options with constant values.
    :return:
    """
    cmt_options = straxen.get_corrections.get_cmt_options(context)

    p = context.get_single_plugin(cmt_run_id, 'peak_positions')
