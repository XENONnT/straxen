...
def url_version_check(url: str):
    if not ("version" in url):
        warnings.warn("A URL without a 'version' argument was given, as a result, to use a url protocol to get a correction a version of said correcection is requiered")
...
def tf_dict_attributes(url: str):
    ...
    if not ("value_attr" in url):
        warnings.warn("Warning, you are requesting a correction in the form of a dictionary with this URL however, you did not include [value_attr] in your url which is needed for dictionry outputs. Please insert [key_attr=] in your url for the keys.")
