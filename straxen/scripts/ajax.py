...
def main():
        ...
        for thread in threads:
            if thread_prefix in thread.name:
                ...
                while wait:
                    ...
                    if thread.isAlive():
                        log.info(
                            f"{thread.name} still running take a "
    ...
    def run():
        ...
        while True:
            try:
                ...
                log.error(
                    f"Fatal warning:\tran into {fatal_error}. Try logging error and restart ajax"
                )
                try:
    ...
    def clean_ceph():
        ...
        If found, delete it. Recursively iterate until no old data is found. This function only
        works on the the host that is eb_can_clean_ceph.
        ...
        if rd is None:
            ...
            remove_run_from_host(
                run_number, delete_live=True, force=args.force, reason="clean ceph"
            )
    ...
    def clean_high_level_data():
        """Check the runs database for old data on this host and clean all non-low-level data-types
        from this host."""
    ...
    def clean_old_hash():
        ...
        for f in files:
            ...
            if current_st_hash != lineage_hash:
                ...
                if rd:
                    delete_data(
                        rd, loc, data_type, test=not args.production, reason="old lineage hash"
                    )
                else:
                    log.info(
    ...
    def _rmtree(path):
        ...
        function in order to make sure that the args.production statement is always double (tripple)
        checked before deleting data.
    ...
    def threaded_delete_data(
        rd, path, data_type, test=True, ignore_low_data_check=False, reason=""
    ):
        ...
        :param ignore_low_data_check: ignore the fact that this might be the only copy of the data.
            We can specify this e.g. if we know this is data associated to some abandoned run.
        ...
        delete_thread.start()
        log.info(
            f"DeleteThread {path} should be running in parallel, continue MainThread now: {now()}"
        )
                ...
                if confirm("update rundoc?"):
                    run_coll.update_one(
                        {"_id": rd["_id"]},
                        {
                            ...
                            "$pull": {
                                "data": {"type": data_type, "host": {"$in": ["daq", hostname]}}
                            },
                        },
                    )
            else:
    ...
    def remove_run_from_host(number, delete_live=False, force=False, reason=""):
        ...
        rd = run_coll.find_one(
            {"number": number, "data.host": hostname if not delete_live else "daq"}
        )
        if not rd:
            log_warning(
                f"No registered data for {number} on {hostname}",
                run_id=f"{number:06}",
                priority="info",
            )
            return
        ...
        for ddoc in rd["data"]:
            ...
            if "host" in ddoc and ddoc["host"] == hostname:
                ...
                if not delete_live:
                    log.info(
                        f'prevent {ddoc["location"]} from being deleted. Do so with --delete_live'
                    )
    ...
    def remove_if_unregistered(number, delete_live=False, clean_from=pre_folder):
        ...
        if rd and any(
            ddoc.get("location", "").startswith(clean_from) for ddoc in rd.get("data", [])
        ):
