...
class S2Width(strax.CutPlugin):
    ...
    def s2_width_model(self, drift_time):
        """Diffusion model."""
        return np.sqrt(
            2 * self.diffusion_constant * (drift_time - self.DriftTimeFromGate) / self.v_drift**2
        )
    ...
    def normWidth(self, events):
        return (
            np.square(events["s2_range_50p_area"] / self.SigmaToR50) - np.square(self.scw)
        ) / np.square(self.s2_width_model(events["drift_time"]))

    def logpdf(self, events):
        return chi2.logpdf(
            self.normWidth(events) * (self.nElectron(events) - 1), self.nElectron(events)
        )
...
class S1SingleScatter(strax.CutPlugin):
    ...
    def cut_by(self, events):
        ...
        alt_rel_width = np.square(
            events[mask]["s2_range_50p_area"] / self.s2width.SigmaToR50
        ) - np.square(self.s2width.scw)
        alt_rel_width /= np.square(
            self.s2width.s2_width_model(self.s2width, events[mask]["alt_s1_interaction_drift_time"])
        )

        alt_interaction_passes = (
            chi2.logpdf(alt_rel_width * (alt_n_electron - 1), alt_n_electron) > -20
        )
...
class S2SingleScatter(strax.CutPlugin):
    ...
    def cut_by(self, events):
        ...
        arr = np.all(
            [
                largest_other_s2_is_nan
                | (events["alt_s2_area"] < self.other_s2_bound(events["s2_area"]))
            ],
            axis=0,
        )
        return arr
...
class S2AreaFractionTop(strax.CutPlugin):
    ...
    def cut_by(self, events):
        arr = np.all(
            [
                events["s2_area_fraction_top"] < self.upper_limit_s2_aft(events["s2_area"]),
                events["s2_area_fraction_top"] > self.lower_limit_s2_aft(events["s2_area"]),
            ],
            axis=0,
        )
        return arr
