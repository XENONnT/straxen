...
def pax_to_records(
    ...
    for event in mypax.get_events():
        ...
        records = np.zeros(n_records_tot, dtype=strax.raw_record_dtype(samples_per_record))
        ...
        for p in event.pulses:
            ...
            for rec_i in range(n_records):
                ...
                r["time"] = event.start_time + p.left * 10 + rec_i * samples_per_record * 10
                r["channel"] = p.channel
...
@strax.takes_config(
    ...
    strax.Option(
        "samples_per_record",
        default=strax.DEFAULT_RECORD_LENGTH,
        track=False,
        infer_type=False,
...
class RecordsFromPax(strax.Plugin):
    ...
    def iter(self, *args, **kwargs):
        ...
        for file_i, in_fn in enumerate(pax_files):
            if self.config["stop_after_zips"] and file_i >= self.config["stop_after_zips"]:
                break
            for records in pax_to_records(
                ...
                yield self.chunk(start=last_endtime, end=new_endtime, data=records)
