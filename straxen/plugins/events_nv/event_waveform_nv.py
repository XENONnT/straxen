...
class nVETOEventWaveform(strax.Plugin):
    """
    Plugin which computes the summed waveform as well as some
    shape properties of the NV events.
    """
    __version__ = '0.0.1'
    ...
    def compute(self, events_nv, records_nv, start, end):
        ...
        strax.copy_to_buffer(
            _tmp_events, events_waveform, '_temp_nv_evts_cpy')
...
def veto_event_waveform_dtype(
    n_samples_wf: int = 200, ) -> list:
    dtype = []
    dtype += strax.time_dt_fields  # because mutable
    dtype += [(('Waveform data in PE/sample (not PE/ns!)', 'data'), np.float32, n_samples_wf),
...
def _temp_event_data_type(n_samples_wf: int = 200, 
                         n_pmts: int = 120,
                         n_widths: int = 11,
                        ) -> list:
    ...
    dtype += [(('Dummy, total area of all hitlets in event [pe]', 'area'), np.float32,),
        (('Dummy top waveform data in PE/sample (not PE/ns!)', 'data_top'), np.float32, n_samples_wf),
