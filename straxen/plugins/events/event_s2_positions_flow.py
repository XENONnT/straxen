...
class EventPositionContour(strax.Plugin):
    ...
    def infer_dtype(self):
        ...
        for type_ in ptypes:
            dtype += [
                (
                    (f"Naive flow position contour for {infoline[type_]}", f"{type_}_position_contour_flow_naive"),
                    np.float32, (self.n_poly+1, 2),
                )
            ]
        ...
        for event_i, (event, sp) in enumerate(zip(events, split_peaks)):
            for type_ in ["s2", "alt_s2"]:
                ...
                if type_index != -1:
                    ...
                    result[f"{type_}_position_contour_flow_naive"][event_i] = sp["position_contours_flow"][type_index]
                ...
                if type_ == "s2":
                    if self.use_fdc_for_contour:
                        ...
                        result["position_contour_flow"][event_i] = sp["position_contours_flow"][type_index]*scale
...
class EventPositionUncertainty(strax.Plugin):
    ...
    def infer_dtype(self):
        ...
        for type_ in ptypes:
            dtype += [
                (
                    (f"Naive flow position uncertainty in r for {infoline[type_]}", f"{type_}_r_position_uncertainty"),
                    np.float32, 
                ),
                (
                    (f"Naive flow position uncertainty in theta for {infoline[type_]}", f"{type_}_theta_position_uncertainty"),
                    np.float32, 
                ),
            ]
    ...
    def compute(self, events):
        ...
        for type_ in ["s2", "alt_s2"]:
            ...
            theta_array = np.arctan2(events[f'{type_}_position_contour_flow_naive'][...,1], 
