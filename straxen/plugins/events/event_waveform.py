...
class EventWaveformS1(strax.Plugin):
    ...
    def infer_dtype(self):
        ...
        for item in self.hits1_dtype:
            ...
            dtype = dtype + [
                ((f'{_comment} in event level', f's1_{_name}'), _dtype, (self.hit_number_peak_waveform_s1))]
        return dtype
...
class EventAmbienceS1(strax.Plugin):
    ...
    def infer_dtype(self):
        dtype = strax.time_fields + [
            (('Lone hits time in event level', 's1_ambience_lone_hits_time'), np.int64,
             (self.hit_number_peak_ambience_s1)),
            (('Lone hits area in event level', 's1_ambience_lone_hits_area'), np.float64,
             (self.hit_number_peak_ambience_s1)),
            (('Lone hits channel in event level', 's1_ambience_lone_hits_channel'), np.int16,
             (self.hit_number_peak_ambience_s1))]
        return dtype
...
class EventWaveformS2(strax.OverlapWindowPlugin):
    ...
    peaklet_number_event_waveform_s2 = straxen.URLConfig(default=20, type=int,
    ...
    def infer_dtype(self):
        dtype = strax.time_fields + [
            (('Center time of the peaklets in S2', 's2_peaklet_center_time'), np.int64,
             (self.peaklet_number_event_waveform_s2)),
            (('Area of the peaklets in S2', 's2_peaklet_area'), np.float32, (self.peaklet_number_event_waveform_s2)),
        ]
        return dtype
...
class EventAmbienceS2(strax.OverlapWindowPlugin):
    ...
    peak_number_event_ambience_s2 = straxen.URLConfig(default=30, type=int,
    ...
    def infer_dtype(self):
        peak_number = self.peak_number_event_ambience_s2
        dtype = strax.time_fields + [
            (('S2 Ambience Peak range_50p_area in event level', 's2_ambience_peak_range_50p_area'), np.int64,
             (peak_number)),
            (('S2 Ambience Peak range_90p_area in event level', 's2_ambience_peak_range_90p_area'), np.float64,
             (peak_number)),
            (('S2 Ambience Peak area_fraction_top in event level', 's2_ambience_peak_area_fraction_top'), np.float64,
             (peak_number)),
            (
            ('S2 Ambience Peak center_time in event level', 's2_ambience_peak_center_time'), np.float64, (peak_number)),
            (('S2 Ambience Peak max_pmt_area in event level', 's2_ambience_peak_max_pmt_area'), np.float64,
             (peak_number)),
    ...
    def fill_result(self, result, split_peaks, events, dtype):
        for i, (_split_peaks, _events) in enumerate(zip(split_peaks, events)):
            ...
            peaks_head = _split_peaks[:self.peak_number_event_ambience_s2]
            for name in dtype:
                if name == 'type':
                    constant_values = -1
                else:
                ...
                result[i][f's2_ambience_peak_{name}'] = np.pad(peaks_head[name],
                                                               (
                                                               0, self.peak_number_event_ambience_s2 - len(peaks_head)),
