...
class AqMonChannelOccupancy:
    ...
    Determines the meaning of the Acquisition Monitor Channels originating from the V1495
    depending on its config in the DAQ. These are start and stop signals of veto- or other
    intervals. The channels have a name according to its primary usage, but the meaning can differ.
      * 'Busy' channels always only provide the busy veto
      * 'Neutron Generator' (NG) channels can provide LED-trigger or Neutron Generator trigger/indicator (or nothing),
                            and for backwards compatibilty (older runs) 'busy_he'
      * 'High Energy Veto' (HEV) channels can provide HEV, HEV-tag, Fractional Lifetime veto, Anti Veto

      HEV-tag means, that we have the information of the HEV, whether it makes a veto decision or not, but we DO NOT
      veto the system.
      More info: https://xe1t-wiki.lngs.infn.it/doku.php?id=xenon:xenonnt:robingb:v1495_firmware_upgrade_v10
    """
    
    neutron_generator_channel_options = {1: "LED_trg",
                                         2: "neutron_generator",
                                         3: "busy_he",  # backwards compatibilty
                                         4: "empty"
                                        }
    
    hev_channel_options = {1: "anti_veto",
                           2: "fractional_lifetime",
                           3: "hev",
                           4: "hev_tag"
                          }
    ...
    def get_v1495_config(self):
        ...
        self.config = {k: (bool(v) if k.startswith(("is_", "_use_")) else v) for k, v in v1495_options.items()}
    ...
    def extract_settings(self):
        ...
        self.use_led_trg = ((self.config.get("_use_regular_port_trg", False) or
                             self.config.get("_use_legacy_port_trg", False)
                            ) and
                            self.config.get("is_led_start_stop_activ", False)
                           )
    ...
    def _settings_check_plausilble(self):
        """
        Check if V1495 config makes sense. Throw error otherwise.
        Redax should not allow most of these, but better be safe than sorry.
        """
        if self.fracLT_on:
            if not ((self.fracLT_veto_on_period > 0) and (self.fracLT_veto_off_period > 0)):
                raise RuntimeError("Fractional Lifetime Veto is set to on, but at least one of its periods is 0.")
        ...
        if self.use_ng_input and self.use_led_trg:
            raise RuntimeError("Neutron generator and LED Trigger are on. This should not be allowed")
        if self.fracLT_on and self.hev_on:
            raise RuntimeError("Fractional Lifetime mode and HEV are on. This should not be allowed.")
    ...
    def reconstruct_anti_veto_from_ng(self):
        ...
        HEV and Anti-Veto would othenwise be on the same AQMon channel.
        In principle we can always do this, once we tested that the reconstruction works.
        """
