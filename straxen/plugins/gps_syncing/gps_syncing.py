...
class GpsSync(strax.OverlapWindowPlugin):
    ...
    def match_gps_pulses_with_truth(gps_pulse_time, gps_truth_time, vicinity):
        ...
        rundb with the corresponding GPS pulse recorded by the acquisition
        monitor.
        ...
        :param vicinity: +/- vicinity in [s] for which GPS pulse is
    ...
    def compute_time_array(self, l_daq_sync, l_gps_sync, l_daq_evt):
        ...
        """
        _has_no_gps_but_events = ((len(l_daq_sync) == 0 
                                   or len(l_gps_sync) == 0)
                                  and len(l_daq_evt))
        if _has_no_gps_but_events:
            raise ValueError(f'Cannot compute GPS correction for "{self.run_id}". '
                             f'There were {len(l_gps_sync)} GPS found in the run-docs, '
                             f'which are marching recorded GPS pulses, '
...
class GpsSync_nv(GpsSync):
    ...
    dtype = strax.time_fields + [(('GPS absolute time [ns]', 'time_sync_gps'), np.int64),]
