...
class EventPeaks(strax.Plugin):
    ...
    def compute(self, events, peaks):
        ...
        """
        sorted_indices_split_peaks_ind = np.argsort(split_peaks_ind) # Sort the indices of fully contained peaks for mapping
        mapping = {val: events['event_number'][i] for val, # Create a mapping dictionary to match event numbers in peaks 
                   ##to event numbers in events
                   i in zip(np.unique(split_peaks_ind[sorted_indices_split_peaks_ind][split_peaks_ind[sorted_indices_split_peaks_ind]!=-1])
                            , range(len(events['event_number'])))}
        # Correct event numbers of peaks based on the mapping dictionary
        corrected_split_peaks_ind = np.array([mapping[val] if val in mapping else val for val in split_peaks_ind]) ##  we run on all numbers in split_paeaks_ind and call them val
