...
class NVDarkRateMonitoring(strax.Plugin):  
    """  
    Plugin to monitor the dark rate, the baseline 
    and the baseline RMS of the Neutron Veto PMTs.  
    ...
    def infer_dtype(self):  
        ...
        dtype = strax.time_fields + [  
            ...
            (
                ('Baselines means mean [ADCc]', 'baselines_means_mean'), 
                np.float32
            ),  
    ...
    def compute(self, raw_records_coin_nv):
        ...
        # Data selection 
        mask = (raw_records_coin_nv['record_i'] == 0) & (
            raw_records_coin_nv['time'] < t_limit
        )  
        ...
        for i, ch in enumerate(self.channels_to_monitor):  
            if ch in count_map:  
                ...
                res['dark_rate_error'][i] = (
                    np.sqrt(n_events) / effective_time
                ) 
