...
def _update_context(st, max_workers):
    ...
    st.set_context_config({"free_options": tuple(st.config.keys()), "forbid_creation_of": ()})
    ...
    for p in plugins:
        for option_name, option in p.takes_config.items():
            ...
            if option.child_option:
                ...
                assert t, (
                    f'This is strange the child option "{option_name}" was set to '
                ...
                assert not t, (
                    f'Found "{parent_name}" in the lineage of {p.__class__.__name__}. '
