...
def test_full_gap_percent_valley(self: PluginTestCase):
    ...
    valley_gap1, valley1 = local_minimum_info.full_gap_percent_valley(
        test_peak_1, max1, min1, 0.9, 1
    )
    valley_gap2, valley2 = local_minimum_info.full_gap_percent_valley(
        test_peak_2, max2, min2, 0.9, 1
    )
    valley_gap3, valley3 = local_minimum_info.full_gap_percent_valley(
        test_peak_3, max3, min3, 0.9, 1
    )
...
def _get_test_peaks():
    ...
    test_peak_1 = np.exp(-((test_time - 75) ** 2) / (2 * std1**2)) + np.exp(
        -((test_time - 125) ** 2) / (2 * std1**2)
    )
    test_peak_2 = np.exp(-((test_time - 75) ** 2) / (2 * std2**2)) + np.exp(
        -((test_time - 125) ** 2) / (2 * std2**2)
    )
    test_peak_3 = np.exp(-((test_time - 50) ** 2) / (2 * std2**2)) + np.exp(
        -((test_time - 150) ** 2) / (2 * std2**2)
    )
