...
from straxen.holoviews.holoviews_tpc_event_display import InteractiveTPCEventDisplay
...
def test_hitlets_to_hv_points():
    ...
    m = [
        hit[key] == points.data[key] for key in hit.dtype.names if key in points.data.columns.values
    ]
...
def test_array_to_df_and_make_sliders():
    dtype = (
        straxen.plugins.events_nv.veto_event_dtype()
...
def test_pmt_array_plot():
    """Test if PMT array can be plotted for the interactive display."""
...
def test_tpc_display_components():
    ...
    event_display = display.event_display(
        inspector_config={"x_bins": (50, 0, 200), "y_bins": (50, 0, 200), "y_field": "s1_rise_time"}
    )

    with TemporaryDirectory() as d:
        event_display.save(os.path.join(d, "display_test.html"))
...
def test_inspector_plot():
    ...
    with TemporaryDirectory() as d:
        plot.save(os.path.join(d, "inspector_plottest.html"))
    ...
    inspector = EventStats(
        ...
        default_config={"x_bins": (10, 0, 10), "y_bins": (10, 0, 10), "y_field": "s1_rise_time"},
    )
    plot, widget = inspector.interactive_event_stats()
